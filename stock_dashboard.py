...
class TechnicalAnalyzer:
    ...
    def _fetch_pe(symbol: str) -> Optional[float]:
        """直接用股票代码查询 PE，不再依赖 df.name（DataFrame 本来就没有这个属性，之前一直返回 None）"""
        if not symbol:
            return None
        try:
...
def safe_fetch(ticker: str, period: str = "3mo") -> pd.DataFrame:
    """独立的行情获取工具函数（修复：原文件中被调用但从未定义，导致 NameError 崩溃）"""
    try:
        df = yf.download(ticker, period=period, progress=False, auto_adjust=True)
        if df.empty:
            return pd.DataFrame()
...
def get_stock_put_call_ratio(symbol: str, api_key: str) -> Tuple[Optional[float], Optional[float]]:
    """
    个股期权 Volume/OI Put-Call Ratio。
    修复说明：Alpha Vantage 并没有 PUT_CALL_RATIO 这个接口（之前代码调用的是不存在的函数名，
    一直返回空），这里改用免费版可用的 HISTORICAL_OPTIONS，自己汇总 Call/Put 的成交量和未平仓量算比率。
    仅支持美股（港股期权 Alpha Vantage 不覆盖，.HK 直接跳过）。
    免费额度每天 25 次请求，谨慎控制调用频率。
    """
...
class MacroCollector:
    ...
    def collect_all(self) -> Dict[str, Any]:
        ...
        # 注：Volume/OI PCR 已改为个股层面展示（见 get_stock_put_call_ratio），
        # 原来这里调用的 Alpha Vantage PUT_CALL_RATIO 是不存在的接口，已移除。
        return macro
...
class SentimentEngine:
    ...
    def calculate(vix: Any, vol_pcr: Any, sox_rsi: Any) -> SentimentResult:
        ...
        label = ("极度贪婪" if composite > 75 else "贪婪" if composite > 55 else
                 "中性" if composite > 45 else "恐惧" if composite > 25 else "极度恐惧")
...
class NewsFetcher:
    ...
    def fetch_baidu(self, query: str) -> List[Dict]:
        ...
        try:
            ...
            results = GoogleSearch({"engine": "baidu_news", "q": query, "api_key": self.cfg.serpapi_key, "num": 3}).get_dict()
...
"""
    ...
    def generate_weekly_outlook(self, macro: Dict, sox: SOXSignals, sp500: Dict, stock_dict: Dict[str, StockTechData],
        ...
        """
        每周总结 + 下周走势展望：综合基本面（PE、宏观利率/流动性）、消息面（新闻）、技术面（均线/RSI/MACD结构）。
        建议每周只跑一次（见 ReportOrchestrator.run 里的星期判断），避免每天都生成一份"周报"没有意义。
        """
        tech_summary = []
        for sym, data in stock_dict.items():
            if not data:
...
class ReportOrchestrator:
    ...
    def _fetch_stock_parallel(self) -> Tuple[Dict[str, StockTechData], Dict[str, Any]]:
        ...
        def worker(sym: str) -> Tuple[str, Optional[StockTechData], Optional[Dict]]:
            try:
                ...
                # 修复：把 symbol 传进去，_fetch_pe 才能真正查到 PE（之前一直是 None）
                ...
                # 新增：个股 Volume/OI Put-Call Ratio（原来market级的Alpha Vantage接口不存在，已改为个股级）
    ...
    def run(self):
        ...
        if self.ai.client:
            ...
            # 每周总结：只在周五（工作日跑批时的收盘后）生成一次，避免每天都刷一份没意义的"周报"
