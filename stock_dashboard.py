# ====================================================================
# 每日股票看板 - 最终版（含图表）
# 使用 Vertex AI (Gemini) + Google Cloud 赠金
# 项目 ID: fast-ability-504013-p6
# ====================================================================

# 1. 安装依赖（包含绘图库）
!pip install -q yfinance pandas ta google-generativeai requests akshare mplfinance matplotlib
...
PROJECT_ID = "fast-ability-504013-p6"   # 你的项目 ID
FRED_API_KEY = "75063c7902071c1eaf1b8453f39198f0"  # 可选
...
print("🔧 正在初始化 Vertex AI Client...")
client = genai.Client(
    vertexai=True,
    project=PROJECT_ID,
    location='global',  # 全局端点，避免 429
    http_options=HttpOptions(api_version="v1")
)
print(f"✅ Vertex AI Client 已初始化 (项目: {PROJECT_ID})")
...
def generate_report():
    try:
        ...
        if FRED_API_KEY:
            cpi = fetch_fred("CPIAUCSL", FRED_API_KEY)
            unemp = fetch_fred("UNRATE", FRED_API_KEY)
            fed_bal = fetch_fred("WALCL", FRED_API_KEY)
            macro["CPI"] = cpi if cpi is not None else "无数据"
            macro["失业率"] = unemp if unemp is not None else "无数据"
            macro["美联储资产负债表"] = fed_bal if fed_bal is not None else "无数据"
...
"""
        ...
        # ---- 生成个股 K 线图（在函数内部，能访问 stock_dict） ----
        print("\n📊 正在生成个股 K 线图...")
        for sym, data in stock_dict.items():
            if "error" not in data:
